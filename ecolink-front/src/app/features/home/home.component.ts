...
@Component({
  ...
  styles: [`
    .home-header {
      background: linear-gradient(135deg, #1a4d24 0%, var(--green) 100%);
...
export class HomeComponent implements OnInit {
  ...
  firstName(): string {
    return this.user()?.name?.split(' ')[0] ?? 'Olá';
  }

  ngOnInit(): void {
    const uid = this.auth.getCurrentUserId();
    if (!uid) return;
    this.actSvc.getByUserId(uid).subscribe({
      next: (a: Activity[]) => { this.activities.set(a); this.loadingActs.set(false); },
      error: () => this.loadingActs.set(false)
    });
    this.rankSvc.getByUserId(uid).subscribe({ next: (r: Ranking) => this.myRanking.set(r), error: () => {} });
  }

  medal(pos: number): string {
    return ({ 1: '🥇', 2: '🥈', 3: '🥉' } as Record<number,string>)[pos] ?? '';
  }

  typeColor(type: string): string {
    return ({ SCHEDULE: '#2563eb', COMPLAINT: '#c0392b', RECYCLING: '#2d7a3a', ACHIEVEMENT: '#d97706' } as Record<string,string>)[type] ?? '#8fa593';
  }
}
