...
@Component({
        ...
        <section style="margin-top:24px">
          ...
          <div *ngIf="!loading() && activities().length > 0" class="hist-list">
            <div class="hist-item card" *ngFor="let a of activities()">
              <div class="hist-type-dot" [style.background]="typeColor(a.type)"></div>
              <div class="hist-info">
  ...
  styles: [`
    .pts-header {
      ...
      padding: 52px 18px 24px; color: #fff;
    }
    .pts-eyebrow { font-size: 11px; opacity: .6; text-transform: uppercase; letter-spacing: .07em; }
    .pts-display { display: flex; align-items: baseline; gap: 6px; margin: 8px 0 10px; }
    ...
    .ach-grid { display: grid; grid-template-columns: repeat(3,1fr); gap: 10px; margin-bottom: 4px; }
    .ach-item {
    ...
    .hist-type-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
...
export class PontuacaoComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!uid) return;
    this.actSvc.getUserHistory(uid).subscribe({
      next: (a: Activity[]) => { this.activities.set(a); this.loading.set(false); },
